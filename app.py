...
# ==========================================
# 📡 실시간 데이터 분석 엔진 (퀀트 지표 추가)
# ==========================================
def get_market_data(ticker_code):
    try:
        tk = yf.Ticker(ticker_code)
        # RSI(14일)와 거래량(20일)을 구하기 위해 과거 2달치 데이터 호출
        ...
        if df is not None and len(df) >= 20:
            ...
            vol_ratio = float(volume / vol_ma20.iloc[-1]) if vol_ma20.iloc[-1] > 0 else 1.0
            
            volatility = float((df['High'].iloc[-1] - df['Low'].iloc[-1]) / price)
            
            return {
                "price": int(price), "rate": rate, "volume": int(volume),
                "rsi": rsi_val, "vol_ratio": vol_ratio, "volatility": volatility
            }
...
st.markdown("**[v1.5 업데이트]** 거래량 급증, RSI, 변동성 지표가 스코어링 엔진에 탑재되었습니다.")
...
# ----------------------------------------
with tab1:
    st.info("🔥 상승률과 **20일 평균 대비 거래량 폭증**을 분석하여 확률 높은 종목을 엄선합니다.")
    
    if st.button("🔄 핵심 주도주 정밀 스캔 및 텔레그램 전송", use_container_width=True):
        with st.spinner("빅데이터 모멘텀, 기술적 지표(RSI/거래량) 분석 중..."):
            results = []
            for code, name in TICKERS.items():
                data = get_market_data(code)
                if data and data["price"] > 0:
                    # 퀀트 스코어 공식: 상승률 * 20일 대비 거래량 폭증 비율 (확률 급상승 로직)
                    score = data["rate"] * data["vol_ratio"]
                    
                    # 리스크 및 특징 감지 (ChatGPT 아이디어 반영)
                    risk_flag = "🟢 정상"
                    if data["vol_ratio"] >= 3.0:
                        risk_flag = "🔥 거래량 급증 (+300%)"
                    elif data["rsi"] >= 75:
                        risk_flag = "⚠️ RSI 과열 구간"
                    elif data["volatility"] >= 0.10:
                        risk_flag = "⚡ 변동성 높음"
                    ...
                    results.append({
                        ...
                        "score": score, "risk_flag": risk_flag
                    })
            
            # 스코어 높은 순 정렬 (상승하면서 거래량 터진 종목 우선)
            ...
            if not top5:
                ...
                st.success(f"✅ 정밀 분석 완료! (기준 시간: {now_time}) - 텔레그램 발송 완료")
                
                tg_msg = f"🦅 <b>[AI 퀀트] 정밀 분석 TOP 5</b> ({now_time})\n"
                tg_msg += "──────────────────\n\n"
                ...
                for idx, item in enumerate(top5, 1):
                    ...
                    st.caption(f"📊 특징: {item['risk_flag']} | RSI: {item['rsi']:.1f} | 거래량 20일 평균 대비: {item['vol_ratio']:.1f}배")
                    
                    col1, col2, col3 = st.columns(3)
                    col1.metric("💸 매수가", f"{price:,}원")
                    ...
                    tg_msg += f" 🔹 특징: {item['risk_flag']} (RSI {item['rsi']:.1f})\n"
