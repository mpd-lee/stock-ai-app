...
# ----------------------------------------
with tab1:
    st.info("⏰ 버튼을 누르면 0초 지연 실시간 데이터를 분석하고, **동시에 텔레그램으로도 추천 브리핑을 전송**합니다.")
    ...
    if st.button("🔄 실시간 TOP 5 스캔 및 텔레그램 전송", use_container_width=True):
        with st.spinner("네이버 금융 실시간 데이터를 분석 중입니다..."):
            ...
            if not top5:
                ...
                st.success(f"✅ 0초 지연 분석 완료! (기준 시간: {now_time}) - 텔레그램 봇으로도 전송되었습니다.")
                
                tg_msg = f"🦅 <b>AI 퀀트 실시간 주도주 TOP 5</b> ({now_time})\n"
                tg_msg += "──────────────────\n\n"
                ...
                for idx, item in enumerate(top5, 1):
                    ...
                    st.link_button(f"📈 {item['name']} 차트/호가창 열기", f"https://m.stock.naver.com/item/main.nhn?code={item['code']}")
                    st.divider() 
                    
                    tg_msg += f"{idx}위. <b>{item['name']}</b> ({item['rate']:+.2f}%)\n"
                    tg_msg += f" 💸 현재가: {price:,}원\n"
                    tg_msg += f" 🎯 익절가: {target:,}원 (+5%)\n"
                    tg_msg += f" 🛑 손절가: {stop:,}원 (-3%)\n\n"
                
                st.info("💡 신한증권 '자동감시주문'에 위 [목표가/손절가]를 그대로 입력해두시면 마음 편히 일상을 즐기실 수 있습니다!")
        ...
        with st.spinner("유니버스 전 종목의 위험 징후를 스캔 중입니다..."):
            ...
            if len(alerts_data) > 0:
                ...
                for item in alerts_data:
                    ...
                    st.link_button(f"📉 {name} 차트/호가창 즉시 확인 (대응하기)", f"https://m.stock.naver.com/item/main.nhn?code={item['code']}")
                    st.divider()
